...
// key, region, model, and API path (e.g. "chat/completions", "responses"). Pass the canonical
...
func (provider *BedrockProvider) mantleOpenAIURL(key schemas.Key, region, model, path string) string {
	base := "v1"
	if strings.Contains(model, "gpt-5") || strings.Contains(model, "gemma-4") {
		base = "openai/v1"
	}
	return fmt.Sprintf("%s/%s/%s", provider.endpointBase(key, serviceBedrockMantle, region), base, path)
}
...
func (provider *BedrockProvider) mantleChatCompletions(
	...
	url := provider.mantleOpenAIURL(key, region, schemas.ResolveCanonicalModel(ctx, request.Model), "chat/completions")
...
func (provider *BedrockProvider) mantleResponses(
	...
	region := resolveBedrockRegion(ctx, key, request.Model)
	url := provider.mantleOpenAIURL(key, region, schemas.ResolveCanonicalModel(ctx, request.Model), "responses")
