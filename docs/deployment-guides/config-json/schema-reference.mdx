...
Bedrock keys additionally support a `bedrock_key_config.endpoints` object with per-service AWS endpoint overrides (`runtime`, `control_plane`, `agent_runtime`, `s3`, `mantle`, `dns_suffix`) for VPC endpoints, ISO/China partitions, and FIPS hosts.
