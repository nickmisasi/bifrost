...
<Note>
  ...
  standard providers, it's optional and overrides the default endpoint. For
  AWS Bedrock, `base_url` overrides the `bedrock-runtime` inference endpoint
  only — see [Bedrock custom endpoints](/providers/supported-providers/bedrock#custom-endpoints-vpc-endpoints-iso-and-china-partitions-fips)
